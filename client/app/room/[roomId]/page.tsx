...
export default function RoomPage() {
  ...
  return (
    <div className="h-screen bg-white dark:bg-black dark:text-white">
      ...
      <div className="h-screen max-w-xl mx-auto flex flex-col border border-[#dcdec1] dark:border-[#494944]">
        <div className="hidden md:block">
         <ThemeToggle />
        </div>
        {/* Header */}
        <div className="border-b dark:dark:border-[#494944] bborder border-[#dcdec1] dark:bg-black px-5 py-3 sticky top-0 bg-white flex items-center justify-between">
          <div>
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-700 dark:text-white">{members.join(", ") || "—"}</p>
          </div>
        </div>
          ...
          {messages.map((message, index) => {
            ...
            return (
              <div key={index} className={`flex flex-col ${isSelf ? "items-end" : "items-start"}`}>
                ...
                <div
                  className={`max-w-[75%] px-3.5 py-2 rounded-2xl text-sm ${isSelf
                      ? "bg-black dark:bg-white dark:text-black text-white rounded-br-sm"
        ...
        {/* Input */}
        <div className="bg-white dark:bg-black border-t dark:border-[#494944] border-[#dcdec1] px-3 py-5 flex gap-2 sticky bottom-0 ">
          <input
          ...
          <button
            onClick={sendMessage}
            className="px-4 py-2.5 rounded-lg bg-black dark:bg-white dark:text-black dark:hover:bg-gray-200 text-white text-sm font-medium hover:bg-gray-800 active:scale-[0.98] transition-all cursor-pointer"
          >
            Send
          </button>
        </div>
